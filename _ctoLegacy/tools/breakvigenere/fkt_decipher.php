...
{   # Trefferspektrum anzeigen
    ...
    {   # nur die Hälfte des spektrums nötig... weil Spiegelsymmetrisch $part=2;
        ...
        if($t>=$ab)
        {
             ...
             {
                 $out.= '<img src='.$ks_hilf[pfad].'/image/bar2.gif width='.$width.' height=';
                 $out.= @round($arr2[$t]/max(array_slice($arr2,0,sizeof($arr2)))*$height,0).'>';
             }
             else
             {
                 $out.= '<img src='.$ks_hilf[pfad].'/image/bar.gif width='.$width.' height=';
                 $out.= @round($arr2[$t]/max(array_slice($arr2,0,sizeof($arr2)))*$height,0).'>';
             }
             if($space>0)
             {
                 $out.= '<img src='.$ks_hilf[pfad].'/image/bar.gif width='.$space.' height=0>';
             }
        }
    }
    $out.= '</td></tr>';
