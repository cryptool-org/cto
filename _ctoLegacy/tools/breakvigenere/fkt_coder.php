...
########################################################
## Bibliothek f�r Kodier und Dekodieralgorithmen
########################################################
#$ksbVersion[]=array('Chiffren-Bibliothek','1.81','20060917','Marcel Br�tz');
...
########################################################
$cipherVersion[]=array('Spaltentauschchiffre','1.1','20060501','Marcel Br�tz');
function revers($tpv)
...
{ # Diese Funktion f�gt die f�r die R�cktransformation
...
{ # erzeugt eine Tauschvorschrift aus einem Wort
  ...
  # aus BLA w�rde 201 --> A:pos 2, B:pos 0, L:pos 1
    $obj=str_split($key);
...
########################################################
$cipherVersion[]=array('Playfair-Chiffre','1.0','20060213','Marcel Br�tz');
...
########################################################
$cipherVersion[]=array('Homophone Chiffre','1.0','20041017','Marcel Br�tz');
...
## Porta-Chiffre v1.0
########################################################
$cipherVersion[]=array('Porta-Chiffre','1.0','20040303','Marcel Br�tz');
...
########################################################
## Autokey-Chiffre v1.0, baut auf Vigen�re-Chiffre auf
########################################################
$cipherVersion[]=array('Autokey-Chiffre','1.0','20040502','Marcel Br�tz');
...
########################################################
## Vigen�re-Chiffre v1.0
########################################################
$cipherVersion[]=array('Vigen�re-Chiffre','1.0','20040501','Marcel Br�tz');
function kodieren($orgtxt,$key,$alfa)
{
    // Buchstabenweise Verchiebekodierung
    // monoalfabetisch bei Schl�ssell�nge 1
...
########################################################
$cipherVersion[]=array('Beaufort-Chiffre','1.0','20040511','Marcel Br�tz');
...
## Bacon-Kodierung
########################################################
$cipherVersion[]=array('Bacon-Kodierung','1.2','20060521','Marcel Br�tz');
...
## Morsen v1.1
########################################################
$cipherVersion[]=array('Morse-Code','1.1','20040602','Marcel Br�tz');
...
## Bin�rkodierung v1.0
########################################################
$cipherVersion[]=array('ISO 8859-1 ASCII','1.0','20030210','Marcel Br�tz');
...
########################################################
$cipherVersion[]=array('Monoalphabetische Ersetzung','1.0','20040324','Marcel Br�tz');
...
########################################################
$cipherVersion[]=array('Transponieralgorithmus','1.0','20040412','Marcel Br�tz');
function transponiere($text, $key, $fill)
{
    // Algorithmus f�r Skytale
    if (strlen($text)>0)
    {
        $length=strlen($text);
        ...
        if($fill>0)
        {
            // dieser Teil f�llt die Matrix um so viele Zeichen auf,
            // bis man die Buchstaben genau in ein Rechteck anordnen kann. ($key*$key2)
            // dies ist beim dekodieren n�tig, da man den unteren Teil des Algorithmus
            // wegschmeissen kann, aber so ist der Code wiederverwendbar.
            $rows=abs($length-($key*$key2));
            $txt='';
            for($t=1;$t<=$rows;$t++)
            {
                $part=substr($text,$length-($t*($key-1)),$key-1);
                $txt=$part.' '.$txt;
            }
...
## Hex-Kodierung
########################################################
$cipherVersion[]=array('Hex-Kodierung','1.0','20040412','Marcel Br�tz');
...
## Polybios
########################################################
$cipherVersion[]=array('Polybios','1.0','20060209','Marcel Br�tz');
...
########################################################
## Gronsfeld-Chiffre, abgewandelte Vigen�re-Chiffre
########################################################
...
########################################################
## Larrabee-Chiffre basiert auf Vigen�re-Chiffre
########################################################
$cipherVersion[]=array('Larrabee-Chiffre','1.2','20060602','Frank Lange, Sebastian Rehberg, Marcel Br�tz');
...
###################################################
$cipherVersion[]=array('XOR-Chiffre','1.1','20060327','Marcel Br�tz');
...
{    #Parameter f�r den Schleifendurchlauf
...
## Huffman-Kodierer
########################################################
$cipherVersion[]=array('Huffman-Kodierer','1.1','20060917','Marcel Br�tz');
...
########################################################
$cipherVersion[]=array('SimpleRotor','0.7b','20060611','Marcel Br�tz');
...
{
    // erzeugt ein Alfabet auf f�r eine Multiplikative Chiffre
