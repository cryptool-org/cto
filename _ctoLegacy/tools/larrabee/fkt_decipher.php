...
########################################################
##
## Bibliothek für Analysealgorithmen
## Version 1.6, Marcel Brätz, 26.03.2006
##
########################################################
$ksbVersion[]=array('Analyse-Bibliothek','1.7','20060610', 'Marcel Brätz');
...
{   # wahrscheinlichste Keylänge aus Häufigkeiten der Buchstaben im deutschen Text, nach Friedman
    $kappa=0;
    $n=strlen($text);
    $hauf = hauf($text);
...
{   # Autokorellation liefert Mustertreffer
    ...
    global $ungefiltert;   # true/false für farbliche Kennzeichnung signifikanter Werte
...
{   # gute Werte filtern, für Ausgabe auf 0 oder 100
...
{   # Trefferspektrum anzeigen
    ...
    $out.='<center><table border=0 cellspacing=0 cellpadding=0>';
    $out.= '<tr><td valign=bottom>';

    $am=@(@array_sum($arr2)/@sizeof($arr2));

    for($t=0;$t<sizeof($arr2)/$part;$t++)
    {   # nur die Hälfte des spektrums nötig... weil Spiegelsymmetrisch $part=2;

        if($t>=$ab)
        {
             if(($arr2[$t]-$schw2)>($am*$schw1))
             {
                 $out.= '<img src=image/bar2.gif width='.$width.' height=';
                 $out.= @round($arr2[$t]/max(array_slice($arr2,0,sizeof($arr2)))*$height,0).'>';
             }
             else
             {
                 $out.= '<img src=image/bar.gif width='.$width.' height=';
                 $out.= @round($arr2[$t]/max(array_slice($arr2,0,sizeof($arr2)))*$height,0).'>';
             }
             if($space>0)
             {
                 $out.= '<img src=image/bar.gif width='.$space.' height=0>';
             }
        }
    }
    $out.= '</td></tr>';
...
function keylen($arr)
{   # Keylänge ermitteln (häufigster Abstand der Maxima im Spektrum, Modus)
...
{   # Abstände zwischen Zeichenketten
