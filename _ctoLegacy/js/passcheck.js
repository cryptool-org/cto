...
function chkPass(pwd) {
	...
	if (pwd) {
		...
		for (var a=0; a < arrPwdLen; a++) {
			...
			var done=false;
			for (var b=a; b < arrPwdLen && done==false; b++) {
				if (arrPwd[a].toLowerCase() == arrPwd[b].toLowerCase() && a != b) { nRepChar++; done=true; }
			}
		}
	...
	/* Modify overall score value based on usage vs requirements */
		...
		if (nRepChar > 0) {  // Same character exists more than once
			nScore = parseInt(nScore - ((nRepChar) * 3));
			sRepChar = "- " + ((nRepChar) * 3);
		}
    ...
    if (langu=="Passwort")
		{
		if (nScore >= 0 && nScore < 20) { sComplexity = "Sehr schwach"; }
		else if (nScore >= 20 && nScore < 40) { sComplexity = "Schwach"; }
		else if (nScore >= 40 && nScore < 60) { sComplexity = "Gut"; }
		else if (nScore >= 60 && nScore < 80) { sComplexity = "Stark"; }
		else if (nScore >= 80 && nScore <= 100) { sComplexity = "Sehr stark"; }
		}
    //english
    else
    {
    if (nScore >= 0 && nScore < 20) { sComplexity = "Very Weak"; }
		...
		var langu = document.getElementById("langu").innerHTML;
		//german
    if (langu=="Passwort") sComplexity="Zu kurz";
		oComplexity.innerHTML = sComplexity;
	}
}
