...
function CTOAdfgxAlgorithm(alpha)
{
    ...
    function encodePartA(plaintext, keysquare, key)
	{
		...
		for (var i = 0; i < plaintext.length; i++)
		{
			...
			codtxt += adfgx.charAt(index / 5) + adfgx.charAt(index % 5);
		}
		return(codtxt);
	}
    ...
    function decodePartB(substitutiontext, keysquare)
	{
		...
		{
			keyindex = adfgx.indexOf(substitutiontext.charAt(i)) * 5 + adfgx.indexOf(substitutiontext.charAt(i + 1));
...
function CTOAdfgxGuiAccess ()
{
	var alphabet = "";
    var cryptoclass = new CTOAdfgxAlgorithm(alphabet);
	...
	function genMyKeysquare(nocrypt)
	{
		...
		if (result==true)
		{
			...
			if (nocrypt!=true) adfgxCrypt(false);
		}
		else
		{
			document.getElementById('mymatrix').className = 'ctoformcss-invalid-style ctoformcss-adfgx-matrix-input-size';
			if (document.CTOAdfgxForm1.code[0].checked == true)
			{
	...
	function setAlphabet(click)
	{
		...
		cryptoclass = new CTOAdfgxAlgorithm(alphabet);
    ...
    this.adfgxCrypt = adfgxCrypt;
    function adfgxCrypt(type) //starts encryption
    {
        genMyKeysquare(true);
        ...
        if (type == "encode")
        {
            document.CTOAdfgxForm1.code[0].checked = true;
            document.CTOAdfgxForm1.code[1].checked = false;
        }

        if (type == "decode")
        {
            document.CTOAdfgxForm1.code[0].checked = false;
            document.CTOAdfgxForm1.code[1].checked = true;
        }
        //get value of radiogroup
        var type;
        for (var i = 0; i < document.CTOAdfgxForm1.code.length; i++)
			if (document.CTOAdfgxForm1.code[i].checked)
                type = document.CTOAdfgxForm1.code[i].value;

		var allowedinput="abcdefghiklmnopqrstuvwxyz";
...
//create adfgx object with alphabet and start necessary scripts
var CTOAdfgxScripts = new CTOAdfgxGuiAccess();