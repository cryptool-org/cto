...
function CTOVigenereAlgorithm(alpha)
{
...
function CTOVigenereGuiAccess ()
{
	var alphabet = "";
    var cryptoclass = new CTOVigenereAlgorithm(alphabet);
	...
	function setAlphabet(click)
	{
		...
		cryptoclass = new CTOVigenereAlgorithm(alphabet);
		checkKey();
		vigenereCrypt(false);
	}
    ...
    function checkKey(nocrypt) //check the key
    {
            ...
            if (document.getElementById("uppercase").checked  == false && document.getElementById("lowercase").checked == true && document.getElementById("casesensitiv").checked == false)
                key = key.toLowerCase();
			document.getElementById("key").value=ctolib.cleanInputString(key, alphabet, false);
			if (nocrypt!=true) vigenereCrypt(false);
    }
    ...
    this.vigenereCrypt = vigenereCrypt;
    function vigenereCrypt(type) //starts encryption
    {

        //set value to radiogroup
        if (type == "encode")
        {
            document.CTOVigenereForm1.code[0].checked = true;
            document.CTOVigenereForm1.code[1].checked = false;
        }

        if (type == "decode")
        {
            document.CTOVigenereForm1.code[0].checked = false;
            document.CTOVigenereForm1.code[1].checked = true;
        }
        ...
        var type;
        for (var i = 0; i < document.CTOVigenereForm1.code.length; i++)

            if (document.CTOVigenereForm1.code[i].checked)
                type = document.CTOVigenereForm1.code[i].value;
...
//create vigenere object with alphabet and start necessary scripts
var CTOVigenereScripts = new CTOVigenereGuiAccess();