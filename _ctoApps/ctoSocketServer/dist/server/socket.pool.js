...
var _job = require('../db/job.queue');

var _job2 = _interopRequireDefault(_job);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function SocketPool(nsp, server) {

    var SOCKET_EVENT_MESSAGE = 'message';
    var SOCKET_EVENT_FAIL = 'fail';
    var SOCKET_EVENT_RESULT = 'result';
    var SOCKET_EVENT_RESULT_DISTRIBUTE = 'result.distribute';
    var SOCKET_EVENT_COMPUTE = 'compute';
    var SOCKET_EVENT_CLOSE = 'close';
    var MESSAGE_CLIENT_CONNECT = 'A new client is connected with group ';
    var MESSAGE_CLIENT_CLOSE = 'A client is disconnected of group ';
    var MESSAGE_WELCOME = 'Hello, you are connect with group ';
    var MESSAGE_BYE = 'Ciao, you are disconnect with group ';
    var MESSAGE_NOT_FREE = 'Sorry, this room is not free, now!';
    var MESSAGE_FAIL_SID = 'The data sid is not equal your socket id!';
    var MESSAGE_FAIL_JID = 'Job id not found!';
    var STATUS_DEFINED = 'defined';
    var STATUS_FREE = 'free';
    var STATUS_COMPUTE = 'free';

    var name = nsp.name;
    var jQueue = new _job2.default();
    var currentJob = '';
    var sockets = [],
        pings = [];
    ...
    function sendCount() {
        ...
        try {
            for (var _iterator = sockets[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
    ...
    function sendAll(header, message) {
        ...
        try {
            for (var _iterator2 = sockets[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
    ...
    nsp.on('connection', function (socket) {
        ...
        socket.on(SOCKET_EVENT_MESSAGE, function (msg) {
            return broadcast(msg);
        });
        socket.on('abort', function (obj) {
            if (!checkSocketId(socket, obj.sid)) {
                return;
            }
            if (currentJob._id !== obj.job._id) {
                return;
            }
        ...
        socket.on(SOCKET_EVENT_RESULT, function (obj) {
            if (!checkSocketId(socket, obj.sid)) {
                return;
            }
            //socket.emit('result', {sid: socket.id, datas: datas});
            //console.log(obj);
            var job = jQueue.getJob(obj.job._id);
            obj.datas.forEach(function (obj) {
                job.addData(obj);
            });
        ...
        socket.on(SOCKET_EVENT_RESULT_DISTRIBUTE, function (obj) {
            if (!checkSocketId(socket, obj.sid)) {
                return;
            }
        ...
        socket.on('result.get', function (obj) {
            if (!checkSocketId(socket, obj.sid)) {
                return;
            }
            if (status !== STATUS_COMPUTE) {
                return;
            }
            var job = jQueue.getJob(obj.jobId);
            socket.emit(SOCKET_EVENT_RESULT, { sid: socket.id, job: job, datas: job.getDatas() });
        });

        socket.on('job.create', function (obj) {
            if (!checkSocketId(socket, obj.sid)) {
                return;
            }

            var job = jQueue.createJob();
            job.name = obj.name;
            job.ctxt = obj.codtxt;
            job.mode = obj.mode;
        ...
        socket.on(SOCKET_EVENT_COMPUTE, function (obj) {
            if (!checkSocketId(socket, obj.sid)) {
                return;
            }
    ...
    function getKeyRange(alpha, clientCount) {
        var serverCount = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
    ...
    function getKeyRange2(alpha, clientCount) {
        var serverCount = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
        ...
        while (alpha.length !== total) {
            ...
            try {
                for (var _iterator3 = clients[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
    ...
    function distribute(job, alpha, cObj) {
        ...
        var obj = { cmd: 'distribute', job: job, mode: cObj.mode, begin: '',
            keyValue: cObj.keyValue, maxComputeLen: cObj.maxComputeLen, keyRange: 0, number: 0, alpha: alpha };
        var nObj = cloneObject(obj, sockets.length);
    ...
    function calcBegin(objs, alpha) {
        var count = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;

        if (alpha.length < count) {
            var range = parseInt(count / alpha.length);
            do {
                var number = count / (2 * range);
                var idx = 0;
                for (var n = 0; n < parseInt(number); n++) {
