...
@date	2016.12
*/
export default function CTOLibService() {
	...
	this.setAlphabet = function(params) {
  ...
  function cleanInputString(text, alphabet, ignore) {
		for (var i = 0; i < text.length; i++) {
			var charerror = true;
      for (var j = 0; j < alphabet.length; j++) {
				if (text.charAt(i) == alphabet.charAt(j)) charerror = false;
			}
  ...
  this.increaseKey = increaseKey;
	function increaseKey(key, alphabet) {
        ...
        if (key > alphabet.length - 1) { key = 0; }
        return(key);
    }
  ...
  function niceAlphabetOutput (alphabet) {
	...
	this.genRandKey = function(keychars) {
  ...
  this.uniqueTest = uniqueTest;
	...
	*/
	...
	this.isValid = function(plain, cipher) {
		for (var i = 0; i < plain.length; i++) {
			if (plain.charAt(i) === cipher.charAt((i) % plain.length)) {
				return false;
			}
		}
		return true;
	...
	this.getClash = function(plain, cipher) {
		var chars = [];
		for (var i = 0; i < plain.length; i++) {
			if (plain.charAt(i) === cipher.charAt((i) % plain.length)) {
				chars.push(cipher.charAt((i) % plain.length));
			}
		}
		return chars;
	}
